...
```{r}
baltimore_county <- read_csv("tabula-Baltimore County; Carey, Samantha log OD.csv", col_names = FALSE) |> clean_names() |>
...
A1. The two dates that had the most OD calls were July 14, 2022 and October 4, 2022. There are no days in which there are no overdose calls. There are 365 days in a year (366 including on a leap year) and there are 366 rows, meaning there was at least one call every day. 
...
A2. I asked ChatGPT "show me how to add a column to my dataframe that displays what day of the week each date occurred in rstudio using lubridate and tidyverse". I then used summarise (b/c that us how we do math in R) and divided the calls each day of the week by total calls and then multiplied by 100 to get a percentage.

```{r}
baltimore_county_by_day <- baltimore_county |>
  mutate(day_of_week = wday(date, label = TRUE, abbr = TRUE)) |>
  group_by(day_of_week) |>
  summarise(
    total_calls = n(),
    percent = total_calls / nrow(baltimore_county) *100
    ) |>
...
A3. All of the data locations are not the same, some have precincts and some don't. There are also some commercial buildings listed like a Dunkin' and a McDonald's. However, the top one with 36 ODs, I am confident that is the right answer. 

```{r}
baltimore_county_location <- baltimore_county |>
  group_by(location) |>
...
A4. Is there a reason why the top address has 36 OD cases? What is going on there?
