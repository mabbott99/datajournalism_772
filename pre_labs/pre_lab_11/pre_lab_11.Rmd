...
999 press releases through earlier dates this year with the title, url, text and date. 
```{r}
...
This is an example of how unique works. Unique is case sensitive, so you need to be sure to list out all the words and how they are spelled differently that you are referring to. A way to circumvent this would be to use string to text. 
```{r}
...
We are now actually using unique terms in reference to the data that we are working with.
```{r}
...
It is mostly conjunctions or articles, but Maryland and U.S. is up there, which makes sense. 
...
Yes. With the removal of the conjunctions and articles, we can actually see the terms used the most, such as health, support, project and funding. 
```{r}
unique_words |>
...
Task 11 is essentially task 8 & 9 but with 2021, 2022 and 2023 results all together. 
```{r}
unique_words_2021 |>
  anti_join(stop_words) |>
  group_by(word) |>
...
**Task** Run the following code and describe the results. **Answer**
There are a lot of terms in reference to COVID. Covid 19 is the main term, with 19 pandemic being third and health care & public health being 7 & 8 respectively. 
```{r}
releases |>
  filter(date < '2022-01-01') |>
...
**Task** Run the following code and describe the results. **Answer**
Covid 19 is still high at 4 and health care jumped up to 3, but besides that there are lot more references to specific areas in Maryland like PG and Baltimore City, as well as a lot of reference to project location and the amount of money being allocated (amount included). Amount included could also be in reference to the stimulus check, maybe?
```{r}
releases |>
...
**Task** Run the following code and describe the results. **Answer**
Nothing in reference to COVID is in the top-10. There are a lot of terms in reference to rights. One is human rights, two is equal rights. Nine is rights amendment. There is also a decent amount of reference to health which has been pretty consistent. One term that is interesting to me to see was African American. 
```{r}
releases |>
  filter(date >= '2023-01-01') |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
...
I've looked through all of them a few times, they seem like they all make sense. However, there are a few words that can be both positive and negative. 
```{r}
