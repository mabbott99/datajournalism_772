...
Just reading in data of Maryland by 11 digit zip code with geographic data. 
```{r}
...
dataframe showing us geography data for each zip code in Maryland. There are two zip code columns, though. One that looks like a regualr zipcode and one that shows us the zip code but also includes the Maryland state code number as well. 
```{r}
View(md_zips)
```
...
plotting our data. We can see Chesapeake Bay. 
```{r}
md_zips |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
installing the library tigris which allows us to use line shapefiles
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
data from the counties in Maryland which will also us to map it with our new function tigris.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Setting the state filter as Maryland for when we map it.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Another map of Maryland, but now the untrained eye wouldn't know that Chesapeake Bay splits Maryland because it is no longer showed on the map. This is good for us though b/c that area still belongs to certain zip codes (i.e a recently purchased light tower)
```{r}
md_counties |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Plotting Maryland again but with county and zip code. 
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
reading in foreclosure data and cleaning up the names. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
A map color coding foreclosure notices using different shades of blue. 
```{r}
...
installing the libraries we need for this lab while also installing, or re-installing, my api key since we are using geographic data. 
```{r}
library(tidyverse)
library(sf)
library(janitor)
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We are reading into foreclosure data in Maryland by county and then showing us the number of notices per county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We can now see every county in MD with the number of people in each county which is label 'estimate'. Also, with geometry = TRUE, we now have mapping data which will also us to make map. 
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below. **Answer**
One of the datasets is the dataset from the population of each county, the same dataframe output that we got from the previous question. Md_county_notices is a data frame that tells us that shows us the amount of foreclosures from each county in Maryland. 
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We now cleaned up the data so we can actually join the two dataframes so we'll be able to accurately chart the data. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
With the Maryland population by county dataframe now cleaned, we can join it woth the foreclosure data so we can make a map. We are using left_join to do so and we are joining by county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We now are adding a column and finding the rate of notices of each county's population and then multiplying by 1000. 
```{r}
md_pop_with_foreclosures <- md_county_population |>
  left_join(md_county_notices, join_by(county)) |> 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
We are now mapping the data from the joined dataset that we have. The key is showing us the rate of foreclosures in each county via different shades of blue. The lighter the blue, the higher the rate. 
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
Here, we are using the magma color scale which more clearly shows the differences in the rates of foreclosures in each county in Maryland. Charles County is clearly the most alarming county, and Garrett and Montgomery are the two counties that we do not have to really worry much about. 
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
...
This is essentially the same map as the last one we produced but instead this time instead of just using a percentage to give us a broad understanding of which counties are heavily affected by foreclosure notices, we can see the more fine-grained details between each county. 
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
