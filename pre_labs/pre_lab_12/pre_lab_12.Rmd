...
We have a date column that is a character column and then we have a press release column. 
```{r}
...
I think a little memo column would be nice, just to give us a little more context
```{r}
...
There are 122 in total, but it is giving us more than we actually want. For example, it is giving us social media links menu items. We only really want listing items (from about 17 and below) minus some things at the bottom. 
```{r}
...
Now instead of all of the elements being listed as HTML sources, we have them as text tags.
```{r}
...
We have the same information we got in the previous question but now it is listed in an actual dataset.
```{r}
