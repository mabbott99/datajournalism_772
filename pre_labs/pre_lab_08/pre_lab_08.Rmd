...
We are taking Baltimore County's 911 phone call data and then we are creating a new column,naming it month and then we are asking the R to take the data, group it by month and then show me the amount of calls that were made each month. Where it says "label=TRUE", we are asking R to actually label each month by its abbreviation and not just 1 for January, 2 for Februrary, and so on and so forth. Lastly, we are asking R to arrange the data from the greatest amount of calls received in a month to the least amount.
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
I see a bar graph with total calls made over the year by month
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
Now it is going in accending order from the least amount of calls in a month to the most
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
The grey bars in the background are no longer there, or it is more minimal
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
The background is blue and the lines are white, the same theme the Economist uses
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
We are  not changing our data to show calls by day as opposed to the amount of calls in a month
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
Each spike in data is a representation of the amount of calls each day. However, I do not believe is this the best way to represent the data because it is not clean nor aesthetically pleasing
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
Now we can see a label for each date in which each call was made, but it is all scrambled so you cannot read it.
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
We not flipped the dates 45º, so now if you squint and tilt your head then you can actually read the dates.
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
