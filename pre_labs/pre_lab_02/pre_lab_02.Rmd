...
**Answer** we added the select to further filter the data we want to see, but in this case, select serves as a way to see less columns. I.E. why we only see I.D and title here
...
**Answer:** the date is now showing as year, month, date
...
**Answer** It is showing us results from the total amount of expenses at UMD from the first of every month from greatest to least greatest
...
**Answer** I attempted running this 2x but I am not sure what is wrong, but I think it is showing us votes for moore, cox, lashar, wallace, all write-ins, including the percent of votes moore got out of the total amount of voters
--
nvm, I realized I had to click general_22 in the data tab
...
**Answer** this is only showing votes for moore , and then changing it into a whole number
...
**Answer** toupper() function and str_to_title() Function
...
**Answer**
It is showing us the expenditure date, payee name (if availble), address, type, amount committee, purpose, vendor, fund type and comments if available. It also shows the state at the very end.
```{r}
...
**Answer** yes, b/c it is giving us each state as the last code and then the total sum amount. It could be a little more detailed and easier to read if more columns were available, however. 
...
1. how could we see more expenses from different states?

2. how could we see the expenses from soley Virginia? How about West Virginia?

3. How many people donated to Wes Moore?
